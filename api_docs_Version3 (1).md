...
- **画像链路:** 所有岗位画像与人物画像均由 LLM 生成并实时落盘到 Neo4j 图数据库（Job / Student / INTEREST_IN 关系），AI 在问答/匹配时优先从图谱检索上下文，再补充最新的数据库画像，形成 “Neo4j + AI” 组合提示词。
...
- **接口说明:** 
  学生首次登录时的必经节点。学生填写的原始技术栈和 MBTI 结果提交后，后端会将其推入消息队列，由大模型异步生成核心的“12维能力雷达图”和初始人物画像，并将结果写入 Neo4j（Student 节点 + radar 属性）。此接口要求前端必须传入 `is_guaranteed=true`（保证书确认）。
- **Request Body:**
  ```json
  {
...
- **接口说明:** 
  学生在职位详情页右下角唤起的 AI 问答接口。后端会将当前页面的【岗位清洗画像】、Neo4j 中的【Job 节点画像】以及当前用户的【12维人物画像/Student 节点画像】组合成上下文注入 LLM，实现 “图谱+大模型” 联动，精准回答“我符合这个岗位吗？”、“我还需要学什么？”等就业相关问题，并在图谱中记录学生对该岗位的关注关系。
- **Request Body:**
  ```json
  {
...
- **接口说明:** 
  支持平台管理端导入大盘数据，也支持企业自行上传招聘模板 Excel。接口采用异步处理设计，解析出表格中的长文本后，系统会按照“站点+岗位编码”进行去重，然后按行拆分成独立的任务扔进 RabbitMQ 消息队列，由后台 LLM 慢慢进行清洗提取，防止接口超时。清洗后的岗位画像会同步写入 Neo4j（Job 节点 + profile 属性），供后续问答与匹配直接检索。
